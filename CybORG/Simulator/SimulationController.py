...
class SimulationController(CybORGLogger):
    ...
    def __init__(self, scenario_generator: ScenarioGenerator, agents, np_random: RandomNumberGenerator):
        ...
        self.red_message_length = getattr(self.scenario_generator, 'RED_MESSAGE_LENGTH', 16)
    ...
    def send_messages(self, messages: dict = None):
        ...
        for agent, message in sorted(messages.items()):
            ...
            for other_agent in self.get_connected_agents(agent):
                if "red" in other_agent:
                    # Store (sender, message) tuple so RedFlatWrapper can assign fixed slots
                    self.agent_interfaces[other_agent].messages.append((agent, message))
                else:
    ...
    def get_message_space(self, agent) -> gym.Space:
        if "red" in agent:
            msg_space = gym.spaces.Box(-1.0, 1.0, shape=(self.red_message_length,), dtype=np.float32)
        else:
