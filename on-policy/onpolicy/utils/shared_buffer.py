...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if act_space.__class__.__name__ == 'Discrete':
            ...
            # Hierarchical mask for autoregressive MultiDiscrete:
            # flat = action_type[n_a] + subnet_by_action[n_a*n_s] + host_by_action_subnet[n_a*n_s*n_h]
            if hasattr(act_space, 'nvec'):
                nvec = [int(n) for n in act_space.nvec]
            else:
                nvec = [int(n) for n in (act_space.high - act_space.low + 1)]
            mask_size = nvec[0]
            for i in range(1, len(nvec)):
                mask_size += int(np.prod(nvec[:i + 1]))
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, num_agents, mask_size),
                dtype=np.float32)
        else:
            self.available_actions = None

        act_shape = get_shape_from_act_space(act_space)
        log_prob_shape = 1 if act_space.__class__.__name__ == 'MultiDiscrete' else act_shape
